...
"""
ClientLaunch successfully started thread that runs the Heartbeat Daemon
:author: Pete Zybrick
...
class SpotRequestMessageInstanceHeartbeatDaemonStarted(SpotRequestMessageBase):
    """ClientLaunch successfully started thread that runs the Heartbeat Daemon"""
    ...
    def process( self, message ) :
        """ Heartbeat daemon successfully started, update timestamp in SpotRequestItem
            TODO: need to add check to determine the heartbeat daemon has started within a reasonable
            time period after the instance has started.  If the heartbeat daemon can't start, the 
            users' script will never run.
        ...
        try:
            ...
            logger.info( fmt_request_item_msg_hdr( spot_request_item ) + 'process_heartbeat_daemon_started' )
            ts_heartbeat_daemon_started = spot_request_msg.name_value_pairs[ SpotRequestMsg.PAIR_NAME_INSTANCE_HEARTBEAT_DAEMON_STARTED_TIMESTAMP]
            spot_request_row_partial_save( self.spot_request_table_name, spot_request_item, 
                                           {TableSpotRequest.ts_heartbeat_daemon_started:ts_heartbeat_daemon_started,
                                            TableSpotRequest.ts_heartbeat:ts_heartbeat_daemon_started }, 
